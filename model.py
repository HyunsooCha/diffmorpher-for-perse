...
class MultiDiffMorpherPipeline(StableDiffusionPipeline):
    ...
    def train_lora_from_images_in_dataset(self,
                                          dataset_path,
                                          prompt="",
                                          save_lora_dir="./lora",
                                          lora_epochs=5,
        ...
        self.scheduler.set_timesteps(num_inference_steps)           # only for lora.

        train_lora_from_images(data_dir=dataset_path, 
                               prompt=prompt, 
        ...
        def morph(alpha_list, progress, desc):
            ...
            if attn_beta is not None:
                ...
                if save_intermediates:
                    first_image.save(f"{self.output_path}/{0:02d}.jpg")
                ...
                if save_intermediates:
                    last_image.save(
                        f"{self.output_path}/{num_frames - 1:02d}.jpg")
                ...
                for i in progress.tqdm(range(1, num_frames - 1), desc=desc):
                    ...
                    if save_intermediates:
                        image.save(f"{self.output_path}/{i:02d}.jpg")
                    images.append(image)
                ...
                for k, alpha in enumerate(alpha_list):
                    ...
                    image = Image.fromarray(image)
                    if save_intermediates:
                        image.save(f"{self.output_path}/{k:02d}.jpg")
                    images.append(image)
